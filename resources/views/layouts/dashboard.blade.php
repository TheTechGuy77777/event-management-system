...
<head>
    ...
    <title>@yield('title', 'Dashboard — EventPlug')</title>
    ...
    <div class="flex h-full">
        ...
        <aside id="sidebar"
            ...
            <nav id="sidebar-nav" class="flex-1 overflow-y-auto px-4 py-4 pb-10">
                ...
                <p class="text-gray-600 text-xs font-medium uppercase tracking-wider px-2 mb-3">Main</p>

                <a href="{{ route('dashboard.index') }}"
                    class="sidebar-link {{ request()->routeIs('dashboard.index') ? 'active' : '' }}">
                    <i class="fa-solid fa-gauge sidebar-icon"></i>Overview
                </a>

                <a href="{{ route('dashboard.events.index') }}"
                    class="sidebar-link {{ request()->routeIs('dashboard.events.index') ? 'active' : '' }}">
                    <i class="fa-solid fa-calendar-days sidebar-icon"></i>My Events
                </a>

                <a href="{{ route('dashboard.events.create') }}"
                    class="sidebar-link {{ request()->routeIs('dashboard.events.create') ? 'active' : '' }}">
                    <i class="fa-solid fa-circle-plus sidebar-icon"></i>Create Event
                </a>

                <a href="{{ route('dashboard.promo-codes.index') }}"
                    class="sidebar-link {{ request()->routeIs('dashboard.promo-codes*') ? 'active' : '' }}">
                    <i class="fa-solid fa-tag sidebar-icon"></i>Promo Codes
                </a>

                <p class="text-gray-600 text-xs font-medium uppercase tracking-wider px-2 mb-3 mt-6">Account</p>

                <a href="{{ route('dashboard.profile') }}"
                    class="sidebar-link {{ request()->routeIs('dashboard.profile') ? 'active' : '' }}">
                    <i class="fa-solid fa-user sidebar-icon"></i>Profile
                </a>

                <a href="{{ route('dashboard.account') }}"
                    class="sidebar-link {{ request()->routeIs('dashboard.account') ? 'active' : '' }}">
                    <i class="fa-solid fa-building-columns sidebar-icon"></i>Bank Account
                </a>

                <a href="{{ route('dashboard.notifications') }}"
                    class="sidebar-link {{ request()->routeIs('dashboard.notifications') ? 'active' : '' }}">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-y-auto">
            ...
            <header class="sticky top-0 z-30 bg-[#080808]/90 backdrop-blur-xl border-b border-white/5 flex-shrink-0">
                <div class="flex items-center justify-between px-6 h-16">
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('dashboard.notifications') }}"
                            class="relative w-9 h-9 glass rounded-xl flex items-center justify-center text-gray-400 hover:text-amber-400 transition-colors">
                            ...
                            @if ($unreadNotifications > 0)
                                <span
                                    class="absolute -top-1 -right-1 w-4 h-4 bg-amber-400 rounded-full text-black text-xs flex items-center justify-center font-bold">
                                    {{ $unreadNotifications > 9 ? '9+' : $unreadNotifications }}
                                </span>
                            @endif
                        </a>
                        <a href="{{ route('dashboard.events.create') }}"
                            class="btn-gold px-4 py-2 rounded-xl text-black font-semibold text-xs hidden sm:flex items-center gap-2">
            ...
            <!-- Page Content -->
            <main class="flex-1 p-6 lg:p-8 pb-16">
